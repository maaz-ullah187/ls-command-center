...
type ContentFilter = 'all' | 'post' | 'short';
type SortOption = 'date' | 'views' | 'engagement' | 'revenue';
type TimeFilter = 'all' | '7d' | '30d';

export default function XTab({ leads, posts: inputPosts }: XTabProps) {
  ...
  if (inputPosts.length === 0) {
    return (
      <div className="bg-[#1a1d23] rounded-xl border border-gray-700 p-12 text-center">
        <p className="text-gray-500 text-sm">X integration not configured yet</p>
      </div>
    );
  }
  ...
  const typeBadgeColor = (type: ContentPost['type']) => {
    switch (type) {
      case 'post': return 'bg-gray-700 text-gray-200';
      case 'short': return 'bg-gray-600 text-gray-100';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          { label: 'Total Conversations', value: totals.conversations.toLocaleString(), color: 'text-gray-200' },
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-1 bg-[#1a1d23] rounded-lg border border-gray-700 p-1">
          {([
            { key: 'all', label: 'All' },
            { key: 'post', label: 'Post' },
            { key: 'short', label: 'Thread' },
          ] as { key: ContentFilter; label: string }[]).map((f) => (
            <button
              key={f.key}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {posts.map((post) => (
          <div key={post.id} className="bg-[#1a1d23] rounded-xl border border-gray-700 p-5 space-y-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2">
                  <span className={`inline-flex px-2 py-0.5 rounded text-[10px] font-semibold uppercase ${typeBadgeColor(post.type)}`}>
                    {post.type === 'short' ? 'Thread' : post.type}
                  </span>
            ...
            {/* Interactions (X-specific labels) */}
            <div className="grid grid-cols-4 gap-3 pt-3 border-t border-gray-700/50">
              {[
                { label: 'Likes', value: post.likes.toLocaleString() },
                { label: 'Replies', value: post.comments.toLocaleString() },
                { label: 'Retweets', value: post.shares.toLocaleString() },
                { label: 'Bookmarks', value: post.saves.toLocaleString() },
              ].map((m) => (
                <div key={m.label}>
                  <p className="text-[10px] text-gray-500 uppercase">{m.label}</p>
            ...
            {post.dmTrigger && (
              <div className="flex items-center gap-2 pt-2 border-t border-gray-700/50">
                <span className="inline-flex items-center px-2.5 py-1 rounded-lg bg-gray-700/50 border border-gray-600 text-gray-200 text-xs font-medium">
                  {post.dmTrigger}
                </span>
            ...
            <div className="grid grid-cols-4 gap-3 pt-3 border-t border-gray-700/50">
              {[
                { label: 'Leads', value: post.leads.toString(), color: 'text-gray-200' },
      ...
      <div className="bg-[#1a1d23] rounded-xl border border-gray-700 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-700">
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">Highest revenue-generating X posts</p>
        </div>
            ...
            <div key={post.id} className="flex items-center gap-4 px-5 py-3 hover:bg-gray-800/50 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-0.5">
                  <span className={`text-[10px] font-semibold uppercase ${typeBadgeColor(post.type)}`}>
                    {post.type === 'short' ? 'Thread' : post.type}
                  </span>
                  <span className="text-[11px] text-gray-500">{post.date}</span>
